    ...
    methods:{
        ...
        async obtenerAlumnos(){
            ...
            if( this.alumnos.length<1 && this.buscar.length<=0){
                fetch((window.assetBase || '') + `private/modulos/alumnos/alumno.php?accion=consultar`)
        ...
        async eliminarAlumno(alumno, e){
            ...
            alertify.confirm('Elimanar alumnos', `¿Está seguro de eliminar el alumno ${alumno.nombre}?`, async e=>{
                ...
                fetch((window.assetBase || '') + `private/modulos/alumnos/alumno.php?accion=eliminar&alumnos=${JSON.stringify(alumno)}`)
                    .then(response=>response.json())
                    .then(data=>{
