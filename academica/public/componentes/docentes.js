const docentes = {
    ...
    methods:{
        ...
        async guardarDocente() {
            ...
            fetch((window.assetBase || '') + `private/modulos/docentes/docente.php`, {
                method: 'POST',
                ...
                .then(data=>{
                    if(data.msg === 'ok' || data === true) {
                        console.log('Docente sincronizado correctamente con el servidor');
                    } else if(data.msg && data.msg.includes('Duplicate entry')) {
                        console.warn('El docente ya existe en el servidor. Se mantiene la copia local.');
                        alertify.warning(`El docente ya existe en el servidor. Se actualizará automáticamente.`);
                    } else {
                        alertify.error(`Error al sincronizar con el servidor: ${data.msg || data}`);
                    }
                })
                .catch(err => {
                    console.warn('Error de conectividad al sincronizar docente:', err);
                    alertify.warning(`No se pudo sincronizar con el servidor. El docente se guardó localmente.`);
                });
