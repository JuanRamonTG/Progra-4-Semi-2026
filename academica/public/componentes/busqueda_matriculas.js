    ...
    methods:{
        ...
        async obtenerMatriculas(){
            ...
            if( this.matriculas.length<1 && this.buscar.length<=0){
                fetch((window.assetBase || '') + `private/modulos/matriculas/matricula.php?accion=consultar`)
        ...
        async eliminarMatricula(matricula, e){
            ...
            alertify.confirm('Elimanar matriculas', `¿Está seguro de eliminar la matricula ${matricula.codigo}?`, async e=>{
                ...
                fetch((window.assetBase || '') + `private/modulos/matriculas/matricula.php?accion=eliminar&matriculas=${JSON.stringify(matricula)}`)
                    .then(response=>response.json())
                    .then(data=>{
