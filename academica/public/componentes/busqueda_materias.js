    ...
    methods:{
       ...
       async obtenerMaterias(){
            ...
            if( this.materias.length<1 && this.buscar.length<=0){
                fetch((window.assetBase || '') + `private/modulos/materias/materia.php?accion=consultar`)
        ...
        async eliminarMateria(materia, e){
            ...
            alertify.confirm('Eliminar materias', `¿Está seguro de eliminar la materia ${materia.nombre}?`, async e=>{
                ...
                fetch((window.assetBase || '') + `private/modulos/materias/materia.php?accion=eliminar&materias=${JSON.stringify(materia)}`)
                    .then(response=>response.json())
                    .then(data=>{
