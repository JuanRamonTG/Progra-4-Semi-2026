    ...
    methods:{
       ...
       async obtenerDocentes(){
            ...
            if( this.docentes.length<1 && this.buscar.length<=0){
                fetch((window.assetBase || '') + `private/modulos/docentes/docente.php?accion=consultar`)
        ...
        async eliminarDocente(docente, e){
            ...
            alertify.confirm('Elimanar docentes', `¿Está seguro de eliminar el docente ${docente.nombre}?`, async e=>{
                ...
                fetch((window.assetBase || '') + `private/modulos/docentes/docente.php?accion=eliminar&docentes=${JSON.stringify(docente)}`)
                    .then(response=>response.json())
                    .then(data=>{
