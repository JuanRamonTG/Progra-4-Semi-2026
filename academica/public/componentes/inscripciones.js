    ...
    methods:{
        ...
        async cargarAlumnos(){
            ...
            if(!this.alumnos.length){
                try {
                    const resp = await fetch((window.assetBase || '') + `private/modulos/alumnos/alumno.php?accion=consultar`);
        ...
        async cargarMaterias(){
            ...
            if(!this.materias.length){
                try {
                    const resp = await fetch((window.assetBase || '') + `private/modulos/materias/materia.php?accion=consultar`);
                    const data = await resp.json();
                    this.materias = data || [];
                    if(this.materias.length) {
                        try {
        ...
        async cargarMatriculas(){
            ...
            if(this.matriculas.length < 1){
                try {
                    const resp = await fetch((window.assetBase || '') + `private/modulos/matriculas/matricula.php?accion=consultar`);
                    const data = await resp.json();
        ...
        async guardarInscripcion() {
            ...
            if(!idMatricula){
                ...
                if(match){
                    ...
                    try {
                        ...
                        fetch((window.assetBase || '') + `private/modulos/matriculas/matricula.php`, {
                            method: 'POST',
            ...
            fetch((window.assetBase || '') + `private/modulos/inscripciones/inscripcion.php`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
